function epheci = ss_coe_to_eci(refsats)
%% DESCRIPTION:
%
...
% -------------------------------------------------------------------------
% INPUT:
% ---->   refsat(year,month,day,hour,min,sec,a,e,i,raan,aop,nu)
...
mu  = 398600.4415;
n_sats = size(refsats,1);
epheci = zeros(n_sats,12);

%% MAIN ALGORITHM
for ou = 1:n_sats
    epheci(ou,1:6) = refsats(ou,1:6);
    a = refsats(ou,7);
    e = refsats(ou,8);
    i = refsats(ou,9) ;
    OMEGA = refsats(ou,10);
